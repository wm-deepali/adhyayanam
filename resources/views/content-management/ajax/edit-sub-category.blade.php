        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex">
                    ...
                    <a href="{{ route('cm.sub.category') }}" style="height: fit-content;" class="btn btn-secondary">
                        ← Back
                    </a>
                </div>
                <div class="mt-2">
                ...
                <form method="POST" action="{{ route('cm.sub-category.update', $subCat->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Examination Commission</label>
                        <select class="form-select" id="commission_id" name="examination_commission_id" required>

                            <option value="" disabled>Select Commission</option>

                            @foreach($commissions as $commission)
                                <option value="{{ $commission->id }}" {{ $commission->id == $subCat->examination_commission_id ? 'selected' : '' }}>
                                    {{ $commission->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="meta_description"
                            placeholder="Meta Description">{{$subCat->meta_description ?? ''}}</textarea>
