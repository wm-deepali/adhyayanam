            ...
            <div class="card-body">
                <div class="d-flex">
                    <div class="col">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted"> Manage Chapter section here.</h6>
                    </div>
                    @if(\App\Helpers\Helper::canAccess('manage_chapter_add'))
                        <div class="justify-content-end">
                            <a href='{{route('cm.chapter.create')}}' class="btn btn-primary">&#43; Add</a>
                        </div>
                    @endif
                </div>
                <div class="mt-2">
                ...
                <table class="table table-striped mt-5" id="chapter">
                    <thead>
                        <tr>
...
@push('after-scripts')
    <script type="text/javascript">
        $(function () {
            gb_DataTable = $("#chapter").DataTable({
                autoWidth: false,
                order: [0, "ASC"],
                processing: true,
                serverSide: true,
        ...
        function multi_delete() {
            ...
            if (confirm("Are you sure you want to Delete this data?")) {
                ...
                if (id.length > 0) {
                    $.ajax({
                        url: "{{ route('cm.chapter.bulk-delete')}}",
                        method: "get",
                        data: { id: id },
                        success: function (data) {
                            alert(data);
                            $('#chapter').DataTable().ajax.reload();
                        }
                    });
                }
                else {
