...
<div class="bg-light rounded">
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('main-topic.update', $topic->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <input type="text" name="id" value="{{$topic->id}}" style="display:none;">
                <div class="mb-3">
                    <label for="name" class="form-label">Name</label>
                    <input type="text" class="form-control" name="name" id="name" placeholder="Name" value="{{$topic->name ?? ""}}" required>
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="file" class="form-control" name="image" accept="image/*" value="{{$topic->image ?? ""}}" >
                    @error('image')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="alt_tag" placeholder="Alt Tag" value="{{$topic->alt_tag ?? ""}}" required>
                    @error('alt_tag')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
