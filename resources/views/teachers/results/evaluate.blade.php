...
@section('content')
    ...
    <form id="evaluationForm" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            @if($isSubjective)
                                                ...
                                                @if($child->teacher_remarks)
                                                    ...
                                                    <div class="p-2 bg-white border rounded">{{ $child->teacher_remarks }}</div>
                                                    </p>
                                                @endif
                            ...
                            <div style="width:75%; padding-right:15px;">
                                ...
                                @if($ans->question->question_type == 'Subjective')
                                    ...
                                    @if($ans->remarks)
                                        <p class="mt-2">
                                            <strong>Teacher Remarks:</strong>
                                        <div class="p-2 bg-white border rounded">{{ $ans->remarks }}</div>
                                        </p>
                                    @endif

                                    @if($ans->evaluation_file)
                                        <p class="mt-2">
                                            <a href="{{ asset('storage/evaluation_marks/' . $ans->evaluation_file) }}" target="_blank"
                                                class="btn btn-sm btn-info">
                                                View Evaluated File
                                            </a>
                                        </p>
        ...
        <div class="block-card mt-4">
            <div class="box-heading">Submit Evaluation</div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="final_status" class="form-control">
                        ...
                        <option value="under_review" {{ $attempt->status == 'under_review' ? 'selected' : '' }}>
                            Under Review (Submit to Admin)
                        </option>
                    </select>
                </div>

                <div class="col-md-4">
                    <label class="fw-bold">Upload Evaluated File</label>
                    <input type="file" name="final_file" class="form-control">
                </div>

                @if($attempt->final_file)
                    <div class="col-md-4 mt-4">
                        <a href="{{ asset('storage/evaluations/' . $attempt->final_file) }}" class="btn btn-info"
                            target="_blank">
                            View Uploaded File
                        </a>
                    </div>
            ...
            <div class="text-end mt-4">
                <button type="button" onclick="submitEvaluation()" class="btn btn-success btn-lg">
                    ✅ Submit Evaluation
                </button>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="marksForm" enctype="multipart/form-data">
                        @csrf
    ...
    <script>
        ...
        function submitMarks() {
            ...
            fetch("{{ route('teacher.results.assign-marks') }}", {
                method: "POST",
                headers: {
                ...
                .then(data => {
                    if (data.status) {   // <-- FIXED
                        ...
                        alert(data.msg ?? "Error saving marks!");
                    }
                });
    ...
    fetch("{{ route('teacher.save-evaluation') }}", {
        method: "POST",
        headers: {
            "X-CSRF-TOKEN": "{{ csrf_token() }}"
        },
    ...
    .then(data => {
        if (data.status) {
            ...
            alert(data.msg ?? "Failed to save evaluation");
        }
    })
    .catch(err => {
