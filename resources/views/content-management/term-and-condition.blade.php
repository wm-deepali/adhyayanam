        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="container mt-4">
                    <form id="term-form" method="POST" action="{{ route('term.conditions.store') }}"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label for="heading1" class="form-label">Heading</label>
                            <input value="{{ $term->heading1 ?? ""}}" type="text" class="form-control" name="heading1"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="editor" name="description1"
                                style="height: 300px;">{!!$term->description1!!}</textarea>
                        ...
                        @if($term->updated_at)
                            <div class="mt-3 text-muted">
                                <small>
                                    Last updated
                                    @if($term->updater)
                                        by <strong>{{ $term->updater->name }}</strong>
                                    @endif
                                    on {{ $term->updated_at->format('d M Y, h:i A') }}
                                </small>
                            </div>
                        @endif
                    </form>
