                    ...
                    <tbody>
                        @forelse($teachers as $teacher)
                            <tr>
                                ...
                                <td>₹{{ number_format($teacher->transactions()->where('type', 'credit')->sum('amount'), 2) }}
                                </td>
                                <td>₹{{ number_format($teacher->transactions()->where('type', 'debit')->sum('amount'), 2) }}
                                </td>
                                <td>
                                ...
                                <td>
                                    @if(\App\Helpers\Helper::canAccess('manage_teacher_transactions'))
                                        <a href="{{ route('teacher.transactions.index', ['teacher_id' => $teacher->id]) }}"
