...
<div class="col-md-12 mt-4">
    ...
    <div class="custom-accordion-item mb-3">
        ...
        <div class="custom-accordion-header"
             onclick="toggleAccordion('{{ $paperId }}')">

            <span>{{ $details->test_paper_type ?? '' }} Paper</span>
            <span class="arrow" id="arrow_{{ $paperId }}">▼</span>
        </div>

        {{-- BODY --}}
        <div id="collapse_{{ $paperId }}" class="custom-accordion-body">
...
@php
    $testType = $details->type ?? null;
@endphp

@if($testType == 2) {{-- Subject Wise --}}
    <p><strong>Subject:</strong> {{ $testpaper->subject->name ?? 'N/A' }}</p>

@elseif($testType == 3) {{-- Chapter Wise --}}
    <p><strong>Chapter:</strong> {{ $testpaper->chapter->name ?? 'N/A' }}</p>

@elseif($testType == 4) {{-- Topic Wise --}}
    <p><strong>Topic:</strong> {{ $testpaper->topic->name ?? 'N/A' }}</p>
...
function toggleAccordion(id) {
    let body = document.getElementById('collapse_' + id);
